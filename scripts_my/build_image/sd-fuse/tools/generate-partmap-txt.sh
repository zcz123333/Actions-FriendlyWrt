...
if [ -f ${SRC_PARAMETER_TPL} ]; then
    ...
    if [ -n "$USERDATA_PARTITION_ADDR" ]; then
        sed -i "s|<USERDATA_PARTITION_ADDR>|${USERDATA_PARTITION_ADDR}|g" ${DEST_PARAMETER_TXT}
    fi

    USERDATA_PARTITION_SIZE=$(grep "^CMDLINE:" ${SRC_PARAMETER_TPL} | sed 's/.*(rootfs),//g;s/@<USERDATA.*//g')
    if [[ "$USERDATA_PARTITION_SIZE" != "-" ]] && [[ "$USERDATA_PARTITION_SIZE" != CMDLINE* ]]; then
        EXTDATA_PARTITION_ADDR=`printf "0x%08x" $((${USERDATA_PARTITION_ADDR}+${USERDATA_PARTITION_SIZE}))`
        sed -i "s|<EXTDATA_PARTITION_ADDR>|${EXTDATA_PARTITION_ADDR}|g" ${DEST_PARAMETER_TXT}
    fi
fi
